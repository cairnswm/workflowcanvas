...
function Minimap() {
  ...
  const handleMinimapClick = (event: React.MouseEvent<HTMLDivElement>) => {
    ...
    if (nodes.length > 0) {
      const xPos = nodes.reduce((min, node) => Math.min(min, node.position.x), Infinity);
      const xMax = nodes.reduce((max, node) => Math.max(max, node.position.x), -Infinity);
      const yPos = nodes.reduce((min, node) => Math.min(min, node.position.y), Infinity);
      const yMax = nodes.reduce((max, node) => Math.max(max, node.position.y), -Infinity);
  ...
  return (
    <div className="absolute bottom-4 left-[75px] z-50">
      <div className={`bg-white rounded-lg shadow-lg transition-all duration-300 ${isMinimized ? 'w-10 h-10' : 'w-[240px] h-[160px]'}`}>
        <button
